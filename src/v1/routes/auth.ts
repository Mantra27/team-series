...
router.post("/google", (req:any, res:any)=>{
    ...
    const body = req.body;
    const IP = "0.0.0.0";
    User.findOne({email: req.body.wt.cu}).then((user:any)=>{
        //user logged in via google for the first time
        if(!user){
            new User({
                username: req.body.wt.Ad,
                email: req.body.wt.cu,
                ...
                const token = jwt.sign({ip:IP, creation: new Date().getTime(), expires: null, username: req.body.wt.Ad, email: req.body.wt.cu}, jwtKey);
...
async function Login(email:any, password:any, res:any){
    ...
    await User.findOne({email: email}).then(async (user:any)=>{
        if(user){
            if(!user.isEmailVerified) return res.json({status: 404, message:`Email not verified. Check your inbox/spam (${user.email})`});
                        ...
                        bcrypt.genSalt(10, async (err:any, salt:any)=> 
                                bcrypt.hash(setNewUser.password, salt, (err:any, hash:any)=> {
                                    ...
                                    .then(async(value:any)=>{
                                        ...
                                        let mykey = crypt0.createCipher('aes-128-cbc', emailVerificationKey);
                                        let mystr = mykey.update(`{"user": "${user}", "email":"${tothe}"}`, 'utf8', 'hex');
                                        mystr += mykey.final('hex');

                                        //sending the email with the unique url
                                        await mail(tothe, { subject: "Dicot Email Verification", body: proxy +`/auth/verifyme?token=${mystr}` }).then((resolve: any) => {
                                            console.log(resolve);
                                            return res.status(200).send({status: 403, message: "Verification Email has been sent successfully, confirm your email to start using Dashboard."});
                                        }).catch((err: any) => {
